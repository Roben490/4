...
export default function EditDriver() {
    ...
    const [role, setRole] = useState<string>(userRole.Driver);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            if (id) {
                ...
                const response = await editDriver(id, {name, email, role});
                if (response) {
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      ...
      />
      <button type="submit">Save <MdSaveAs color='red'/></button>
    </form>
